 ...
 */
export function routeRequest(estimatedTokens = 1000, skipKeys, preferredModelDbId) {
    const db = getDb();
  ...
  `).all();
    ...
    for (const entry of sortedChain) {
        ...
        const keys = db.prepare("SELECT * FROM api_keys WHERE platform = ? AND enabled = 1 AND status IN ('healthy', 'unknown')").all(model.platform);
        if (keys.length === 0)
            continue;
